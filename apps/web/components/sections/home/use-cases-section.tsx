...
function ScrollButton({
  ...
  return (
    <button
      ...
      className="border-brand-dark-green/15 text-brand-dark-green hover:bg-brand-dark-green hover:text-brand-off-white flex size-10 cursor-pointer items-center justify-center rounded-full border transition-all"
    >
...
function UseCaseCard({ title, description, href, imageSrc, learnMore }: UseCaseCardProps) {
  return (
    <div className="border-brand-dark-green/10 flex h-79.25 w-71.25 shrink-0 flex-col rounded-2xl border p-4 transition-shadow hover:shadow-lg md:w-auto">
      ...
      <div className="mt-3">
        <TextLink href={href}>{learnMore}</TextLink>
      </div>
      <div className="mt-auto flex items-end justify-between gap-4">
        <p className="text-mono-s text-brand-dark-green/70 max-w-46.5">{description}</p>
...
export default function UseCasesSection() {
  ...
  return (
    <section id="use-cases" className="w-full overflow-hidden bg-brand-off-white py-20 md:py-28">
      <div className="mx-auto max-w-354 px-3">
        ...
        <div className="mb-2 flex items-start gap-3">
          <div className="relative h-20.25 w-26.75 shrink-0 overflow-hidden rounded-lg">
            <Image src="/images/home/usecase-1.jpg" alt="" fill className="object-cover" />
          </div>
        </div>
        <h2 className="text-h1 text-brand-dark-green text-center">{t('useCases.title')}</h2>
        ...
        <div className="mt-4 mb-6 flex items-center justify-between">
          ...
          <TextLink href={ROUTES.buildersHub}>{t('useCases.cta')}</TextLink>
        </div>
      </div>
      ...
      >
        {cards.map((card) => (
          <UseCaseCard key={card.title} {...card} learnMore={t('useCases.learnMore')} />
        ))}
      </div>
    </section>
  )
