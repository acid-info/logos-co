...
Guidance for agents working inside `apps/civi-crm-cms`. The root `AGENTS.md` still applies; this file adds app-specific instructions for the nearest subtree.

## App Role

This is the Logos CiviCRM CMS — a Next.js 16 internal tool for managing Circle Case onboarding. It uses Tailwind v4, `@acid-info/logos-ui`, and `@acid-info/logos-tokens`. No database — CiviCRM is the sole data store. Authentication is handled by a Keycloak reverse proxy. Dev server runs on port **3002**.

**Architecture document:** [`docs/civi-crm-cms-architecture.md`](../../docs/civi-crm-cms-architecture.md)
...
When you make a structural change, update both `docs/civi-crm-cms-architecture.md` **and** this `AGENTS.md` in the same commit or PR if either is affected. Changes that require a doc update:
...
```bash
pnpm --filter civi-crm-cms dev
pnpm --filter civi-crm-cms build
pnpm --filter civi-crm-cms lint
pnpm --filter civi-crm-cms lint:fix
pnpm --filter civi-crm-cms check-types
pnpm --filter civi-crm-cms test
```
