...
/**
 * POST /api/content-workflow/save-circle-as-pr
 *
 * Body: `{ id: string }` — the Payload Circle document id.
 *
 ...
 * field-level Zod failures (missing required field, invalid timezone, etc.) to
...
export const POST = async (req: NextRequest): Promise<NextResponse> => {
  ...
  let doc: CircleDocLike
  try {
    doc = (await payload.findByID({
      collection: 'circles',
      id: body.id,
      depth: 0,
    })) as unknown as CircleDocLike
  } catch (err) {
    return NextResponse.json(
      {
        error: `failed to load Circle ${body.id}: ${
          err instanceof Error ? err.message : String(err)
        }`,
      },
      { status: 404 }
  ...
  try {
    const result = await saveCircleAsPullRequest({
      doc,
      payload,
      editor: {
