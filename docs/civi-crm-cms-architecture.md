# `civi-crm-cms` — Architecture Document

> Status: pre-implementation  
> Target mono-repo: `acid-info/logos-co`  
> Node ≥ 24, pnpm 11.1 (pending mono-repo upgrade before implementation)

---

## 1. Overview

`civi-crm-cms` is a Next.js application that provides a lightweight read/write interface into CiviCRM for Logos Circle Case management. It lives in `apps/civi-crm-cms` inside the existing mono-repo, alongside `apps/web` and `apps/cms`.
...
| `/case/[id]` | Read + write | Detail view of a Case and its linked Contact Individual |
...
```
ACTIVE_VIEW=circles_onboarding   # server-only; change to switch the entire app to a different view
```
...
### 5.3 `circles_onboarding` view config (abbreviated)

```typescript
const circlesOnboardingView: ViewConfig = {
  id: 'circles_onboarding',
  label: 'Circles Onboarding',
...
export function getCurrentUserEmail(req: Request): string | null {
  // DEV_USER_EMAIL_MOCK overrides the header in all environments where it is set.
  // Set it to your own email in .env.local during development.
  if (process.env.DEV_USER_EMAIL_MOCK) return process.env.DEV_USER_EMAIL_MOCK
...
`KEYCLOAK_USER_EMAIL_HEADER` controls the header name (default `x-auth-request-email`). `DEV_USER_EMAIL_MOCK` short-circuits header reading entirely and is the development mock. The actual header name Infra will inject remains TBD — see §17.
...
| `components/cases/ActivityLog.tsx` | Client Component | Rendered from server-fetched data; no client-side fetching |
...
ACTIVE_VIEW=circles_onboarding
...
The `civi-crm-cms` app requires no additions to `turbo.json` tasks beyond the existing `build`, `dev`, `lint`, `check-types`, and `test` tasks. Add the app-specific env vars to the `build.env` array:
```json
